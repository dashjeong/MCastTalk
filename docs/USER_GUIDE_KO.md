...
이 안내서는 MCastTalk `0.2.42-alpha`의 사용법을 설명합니다. 새 기능의 사용 조건은 11·12·13절을 확인하세요.
...
1. GitHub 저장소의 [Releases](https://github.com/dashjeong/MCastTalk/releases)에서 검증된 `MCastTalk-0.2.42-alpha.apk` 파일을 다운로드합니다.
...
0.2.41부터 제공하는 실시간 읽기와 복구 기능입니다.
...
## 13. 0.2.42 자동 준비·API·아스트라 미니미

- **설정 → 언어 → 선택 언어 자동 준비**: 원문·대상 언어나 음성 선택을 바꾸면 저장된 자료를 재사용하고 필요한 자료만 준비합니다. 입력·방송·파일 작업 중에는 준비를 미룹니다. 새 설치의 기본 대상은 영어·일본어·중국어 간체/번체·베트남어이며, 기존 선택은 유지됩니다. 최초 자료 준비에는 네트워크·저장 공간이 필요합니다. 자동 준비를 끄거나 진행 중 취소할 수 있고, 실패한 준비는 수동 재시도합니다.
- **설정 → 모델 → 번역 API**: 기기 내 번역, OpenAI, Gemini, OpenAI 호환을 선택합니다. 호환 서버는 HTTPS 기본 주소·모델·Responses 또는 Chat Completions 방식을 지정하세요. 키를 저장한 뒤 온라인 사용을 허용해야 합니다. 목적지·모델·키를 바꾸면 허용을 다시 켜야 합니다. 파일 변환에서는 **설정한 API**를 선택합니다. 준비된 로컬 모델로의 대체 번역을 선택할 수 있습니다.
- **번역 문체**: 자동은 대화/안내 상황에 맞는 문체를 지시합니다. 구어체와 격식체를 직접 고를 수도 있습니다. API/Gemma의 지시와 문장 사전에 적용되며 ML Kit 자체에 문체 제어 기능을 추가하는 것은 아닙니다.
- **설정 → 아스트라 미니미·진단과 개선 제안**: 입력 복구, 언어별 실패, 준비와 지연을 현재 기기 상태에 근거해 안내합니다. 제안 버튼을 눌러 해당 화면으로 이동하며 방송이나 입력을 임의로 중지하지 않습니다.
- **개발자 표시 → 실험실 → 자가진단·자기개선 모드**: 스승 API의 키와 온라인 검토 허용을 별도로 설정합니다. 개선 신호가 있는 문장만 제한된 횟수로 검토하고 **학습 전후 비교 리포트**에서 원문·기존 번역·개선안과 근거를 보여줍니다. **승인·적용 / 보류**를 선택하세요. 승인된 문장은 이후 같은 문장에 사용하며 기존 사용자 확정 문장은 보존됩니다. 모델 가중치를 학습하는 기능은 아닙니다.
- 리포트에서 해당 보정의 적용을 취소하거나 JSON으로 내보낼 수 있습니다. **사전 백업/가져오기**에 리포트도 포함되며 가져온 기록은 비교용입니다. 가져오기만으로 승인이나 삭제 권한이 생기지 않습니다. 리포트는 최대 200개, 문장 사전은 최대 50,000개입니다.
- 자기개선 제안의 설정 변경은 승인 후 적용됩니다. 코드 수정이 필요한 제안은 개발 검토 기록으로 남습니다. 실제 의미 품질과 장시간 실기기 동작은 사용 환경에서 확인해야 합니다.
