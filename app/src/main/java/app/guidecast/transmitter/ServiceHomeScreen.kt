...
internal enum class MCastService(val title: String, val description: String, val outcome: String, val preparation: String) {
    ...
    MULTILINGUAL("실시간 통역", "말소리와 번역을 실시간으로 확인하세요.", "언어 선택 → 통역 준비 → 언어별 채널로 듣기", "최초 모델 준비에는 인터넷·저장 공간이 필요합니다. 원음 방송은 통역 준비 중에도 열 수 있습니다."),
...
internal fun ServiceHomeScreen(activeService: MCastService?, onSelect: (MCastService) -> Unit) {
    Surface(Modifier.fillMaxSize()) {
      BoxWithConstraints {
        ...
        val services = listOf(MCastService.MULTILINGUAL, MCastService.VOICE, MCastService.NOTES, MCastService.FILES)
