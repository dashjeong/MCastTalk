...
class CloudTranslationReviewer internal constructor(
    ...
    suspend fun refine(sourceLanguageTag: String, targetLanguageTag: String, original: String,
        draft: String, live: Boolean = false): String {
        val capturedStyle = currentCoroutineContext()[TranslationStyleContext]
        val register = if (capturedStyle?.style == TranslationStyle.CONVERSATIONAL)
            TranslationRegister.CONVERSATIONAL else TranslationRegister.FORMAL
        ...
        if (saved != null && (saved.origin == SentenceMemoryOrigin.USER || (developerInfo() && options.autoLearnEnabled))) {
            return saved.corrected
        }
        if (!maySend(options)) return draft
        val request = CloudReviewRequest(options.provider, options.modelId, sourceLanguageTag,
        ...
        if (!admission.acquire(key, live)) { mutableStatus.value = CloudReviewStatus.BUSY; return draft }
        if (live) {
            scope.launch {
                try { reviewAndMaybeLearn(request, requireLearning = true) }
                finally { admission.release(key) }
            }
            return draft
        }
    ...
    private fun mayLearn(request: CloudReviewRequest): Boolean {
        val current = settings.state.value
        return maySend(current) && current.autoLearnEnabled &&
            current.provider == request.provider && current.modelId == request.modelId
    }
    ...
    private suspend fun reviewAndMaybeLearn(request: CloudReviewRequest, requireLearning: Boolean): String? {
        ...
        val options = settings.state.value
        if (!maySend(options) || options.provider != request.provider || options.modelId != request.modelId ||
        ...
        val corrected = try {
            withTimeoutOrNull(REVIEW_DEADLINE_MILLIS) { transport.review(request, key) }
                ?.takeIf { conservativeReviewAccepted(request.original, request.draft, it, request.targetLanguageTag) }
        } catch (cancelled: CancellationException) { throw cancelled }
        catch (_: Exception) { null }
        currentCoroutineContext().ensureActive()
        if (corrected == null) { mutableStatus.value = CloudReviewStatus.FALLBACK; return null }
        val latest = settings.state.value
        if (!maySend(latest) || latest.provider != request.provider || latest.modelId != request.modelId) return null
    ...
    companion object { const val REVIEW_DEADLINE_MILLIS = 1_500L }
}
...
internal class OfficialCloudReviewTransport(
    ...
    override suspend fun review(request: CloudReviewRequest, apiKey: String): String? {
        ...
        val response = http.post(endpoint, request.provider, apiKey, payload) { isAuthorized(request) } ?: return null
        return CloudReviewJson.corrected(request.provider, response)
    }
}
...
internal object CloudReviewJson {
    fun request(request: CloudReviewRequest): String {
        ...
        val instructions = "Review a translation from ${request.sourceLanguageTag} to ${request.targetLanguageTag}. " +
            "Use ${request.translationRegister.name.lowercase()} language. " +
    ...
    fun corrected(provider: CloudReviewProvider, response: String): String? = runCatching {
        ...
        if (result.length() != 2 || result.opt("accepted") != true || result.opt("corrected") !is String) return null
        result.getString("corrected").trim().takeIf { it.length in 1..8_000 }
    }.getOrNull()

    private fun strictObject(text: String): JSONObject {
        require(text.length <= BoundedCloudHttps.MAX_RESPONSE_BYTES)
...
internal class BoundedCloudHttps(
    ...
    suspend fun post(endpoint: String, provider: CloudReviewProvider, apiKey: String, body: String,
        ...
        if (!validEndpoint(url, provider) || apiKey.any { it == '\r' || it == '\n' } || !allowedToSend()) return null
        ...
        return suspendCancellableCoroutine { continuation ->
            ...
            val task = FutureTask {
                val response = runCatching {
                    val deadline = System.nanoTime() + CloudTranslationReviewer.REVIEW_DEADLINE_MILLIS * 1_000_000L
                    ...
                    try {
                        ...
                        client.inputStream.use { input ->
                            ...
                            while (continuation.isActive && allowedToSend()) {
                                ...
                                client.readTimeout = minOf(1_000, remaining)
