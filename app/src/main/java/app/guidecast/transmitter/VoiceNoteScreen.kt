        ...
        LazyColumn(contentPadding = PaddingValues(20.dp), verticalArrangement = Arrangement.spacedBy(14.dp),
            ...
            stickyHeader {
                ...
                FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    FilterChip(!showLibrary, onClick = { showLibrary = false }, label = { Text("노트 작업") })
                    FilterChip(showLibrary, onClick = { showLibrary = true }, enabled = !state.busy && !state.recording,
                        label = { Text("보관함 (${state.library.size})") })
                }
                }
            }
                ...
                when {
                    ...
                    note.interrupted -> Button(onClick = model::recover, enabled = enabled) { Text("중단된 녹음 복구") }
                        ...
                        if (note.lines.isNotEmpty()) {
                            ...
                            }, enabled = enabled && (missing > 0 || target != note.targetLanguage), modifier = Modifier.fillMaxWidth()) {
                        ...
                        FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            OutlinedButton(onClick = { if (model.prepareExport("wav")) wav.launch("MCastTalk-${note.id}.wav") }, enabled = enabled && note.durationMs > 0) { Text("녹음 내려받기") }
                        }
                ...
                if (note.lines.isNotEmpty()) {
                    item {
                        ...
                        if (transcriptQuery.isNotBlank()) {
                            ...
                            TextButton(onClick = { transcriptQuery = "" }) { Text("검색 지우기") }
                        }
                    }
                    items(matchingLines, key = { "line-$it" }) { index ->
                        ...
                        Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor =
                            ...
                            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                                ...
                                FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                    TextButton(onClick = { model.playFrom(line.startMs) }, enabled = enabled && !note.interrupted) { Text("여기서 듣기") }
                                    TextButton(onClick = { speakerIndex = index; speaker = line.speaker; speakerAttempted = false }, enabled = enabled) { Text("화자 이름") }
                                    TextButton(onClick = { editIndex = index; editOriginal = line.original; editTranslation = line.translation; translationEdited = false; editAttempted = false }, enabled = enabled) { Text("문장 수정") }
                                }
                            }
                        }
                    }
            ...
            item {
                ...
                if (libraryQuery.isNotBlank()) TextButton(onClick = { libraryQuery = "" }) { Text("제목 검색 지우기") }
                Text("${library.size}개 노트 · 최신 녹음 순 · 최대 500개", style = MaterialTheme.typography.bodySmall)
                OutlinedButton(onClick = { model.leave(); title = ""; showLibrary = false }, enabled = enabled) { Text("새 음성노트") }
            }
            ...
            items(library, key = { it.id }) { saved ->
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(12.dp)) {
                        ...
                        Row {
                            TextButton(onClick = { model.open(saved.id); showLibrary = false }, enabled = enabled) { Text("열기") }
