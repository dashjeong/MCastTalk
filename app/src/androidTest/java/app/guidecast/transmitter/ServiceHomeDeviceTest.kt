    ...
    @Test fun workspaceAndCommonTabNavigationPreserveStandaloneAndAudioPreferences() {
        ...
        try {
            for (translationEnabled in listOf(false, true)) {
                ...
                try {
                    for (service in listOf(MCastService.MULTILINGUAL, MCastService.NOTES, MCastService.FILES)) {
        ...
        try {
            for (translationEnabled in listOf(false, true)) {
                val expected = previous.copy(
                    ...
                    targetLanguageTags = if (translationEnabled) listOf("en") else emptyList(),
                ...
                try {
                    ...
                    assertEquals("Entering unified workspace must preserve translation setting",
                        translationEnabled, app.operatorSettings.state.value.translationEnabled)
                    assertEquals("Entering unified workspace must preserve standalone mode",
                        BroadcastRunMode.STANDALONE, app.operatorSettings.state.value.runMode)
                    ...
                    assertEquals("Stopping broadcast must preserve translation setting",
                        translationEnabled, app.operatorSettings.state.value.translationEnabled)
                    device.pressBack()
                    assertTrue(device.wait(Until.hasObject(By.text("오늘은 무엇을 할까요?")), 5_000L))
                } finally {
            ...
            instrumentation.runOnMainSync { app.operatorSettings.restore(previous) }
        }
    }
